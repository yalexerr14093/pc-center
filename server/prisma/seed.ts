...
const fullReset = process.argv.includes("--reset");
...
async function upsertAdmin() {
  ...
  console.log(`Admin: ${adminEmail} (password from ADMIN_PASSWORD or default "adminadmin")`);
}

async function main() {
  if (fullReset) {
    // eslint-disable-next-line no-console
    console.log("FULL RESET: удаляю заказы и каталог, пересоздаю стартовые товары…");
  ...
  if (n === 0) {
    // eslint-disable-next-line no-console
    console.log("Каталог пуст — добавляю стартовые позиции…");
    await seedCatalog();
  } else {
    // eslint-disable-next-line no-console
    console.log(`Каталог уже есть (${n} товаров) — пропускаю создание стартового каталога.`);
  }

  // eslint-disable-next-line no-console
  console.log("Готово. Полный сброс каталога и заказов: npm run db:seed:reset (из корня репо).");
}
